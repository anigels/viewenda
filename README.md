...
Cue is a mobile-first streaming planner for finding something to watch and making plans together. This is **Phase 1: technical foundation**, with a working Ionic shell and local primary-profile editing. It is not the complete MVP.
...
- `src/features/onboarding/`: primary profile editing and service-selection placeholder.
- `src/features/search/`, `watchlist/`, `plan-tonight/`, `calendar/`: routed feature placeholders.
- `src/pages/`: Home.
...
The TMDB wrapper returns discriminated configuration/HTTP/network errors, uses a 15-second request timeout, and does not log credentials or raw remote errors. Multi-search excludes people. Provider methods require the caller's saved profile region; no UI-wide US assumption is used. Live TMDB responses are not tested without a user-supplied development token. Confirm response contracts and add runtime validation before exposing live search. Add required TMDB/JustWatch attribution when displaying their data.

## Intentionally unimplemented

Live search UI, provider picker, adding/editing watchlist entries, viewer management, nominations and voting, scheduling, calendar UI and automatic event discovery; authentication, cloud sync, conflict resolution, complete storage migrations, native packaging, final icons, deployment and TestFlight automation. Profile name/region saving is the small working vertical slice for the persistence contract.
