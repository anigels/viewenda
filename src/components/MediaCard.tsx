...
export function MediaCard({ media, onOpen, children }: { media: MediaReference; onOpen: () => void; children?: ReactNode }) {
  const poster = posterUrl(media.posterPath);
  return <IonCard className="media-card"><IonCardContent>
    {poster ? <img className="poster" src={poster} alt="" loading="lazy" onError={e => { e.currentTarget.style.visibility = 'hidden'; }} /> : <div className="poster poster-placeholder" aria-hidden="true">V</div>}
