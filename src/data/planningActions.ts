import type { CueData } from './CueRepository';
...
export const allViewers = (data: CueData) => [{ id: data.profile.id, name: data.profile.name }, ...data.viewers];
export const currentNight = (data: CueData) => data.watchNights.find(night => !night.watchPlanId);
export function cleanNight(data: CueData, night: WatchNight) {
  return normalizeNight(night, allViewers(data).map(viewer => viewer.id), data.watchlist.map(item => mediaId(item.media)));
}
export function changeNight(data: CueData, id: string, change: (night: WatchNight) => WatchNight): CueData {
...
export function startNight(data: CueData, id: string): CueData {
...
export function addViewer(data: CueData, id: string, name: string): CueData {
...
export function removeViewer(data: CueData, id: string): CueData {
...
export function scheduleNight(data: CueData, nightId: string, planId: string, date: string, time: string): CueData {
...
export function saveManualPlan(data: CueData, plan: WatchPlan): CueData {
...
export function reschedulePlan(data: CueData, id: string, date: string, time: string): CueData {
  requireSchedule(date, time);
...
export function removePlan(data: CueData, id: string): CueData {
